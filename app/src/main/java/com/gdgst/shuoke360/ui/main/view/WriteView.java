...
public class WriteView extends SurfaceView implements
     ...
     * 默认手绘笔迹粗细
     */
    private  int DEFAULT_HANDWRTING_STROKE_WIDTH = PractiseActivity.getPaintSize();
    ...
    private  String SRC_XINGKAI_FONT = PractiseMainFragment.getChioceFonts();
    ...
    private void initPaint() {
        ...
        // 初始化手绘画笔
//        Log.v("zhongmo_handwritingPaint", DEFAULT_HANDWRTING_STROKE_WIDTH+"");
        ...
        handwritingPaint.setStrokeJoin(Paint.Join.ROUND);
        handwritingPaint.setColor(DEFAULT_PEN_COLOR); // 设置画笔颜色
        handwritingPaint.setStrokeCap(Paint.Cap.ROUND);
        handwritingPaint.setPathEffect(new CornerPathEffect(50));
    ...
    private void initView(Context context) {
        ...
        if (SRC_XINGKAI_FONT == null){
            SRC_XINGKAI_FONT = "fonts/xingkai.ttf";
        }
...
//                 path.reset();
    ...
    public  void saveBiatmap(){
        String path = Environment.getExternalStorageDirectory().getAbsolutePath()+"writeview"+System.currentTimeMillis()+".png";
        if (!new File(path).exists()){
            new File(path).getParentFile().mkdir();
        }
        saveBitampByBng(getBitmap(), path);
    }
    ...
    private void doDraw(Canvas canvas){
        ...
        penDraw(canvas);
        canvas.drawBitmap(mBitmap, 0, 0, handwritingPaint);
    }

    private void saveBitampByBng(Bitmap bitmap, String filePath){
